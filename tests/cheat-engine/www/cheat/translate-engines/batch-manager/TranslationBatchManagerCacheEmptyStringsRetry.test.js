...
describe('TranslationBatchManager cacheEmptyStrings repeat behavior', () => {
    it('retries only unresolved items and keeps successful ones applied', async () => {
        ...
        const result = await manager.runBatchedTranslation([
            {
        ...
        expect(runtime.engine.seenBatchValues[0]).toEqual([
            'Source message',
            'Source description',
        ]);
        expect(runtime.engine.seenBatchValues[1]).toEqual(['Source description']);

        expect(runtime.translationCache.get('message:ja-en-Source message')).toBe('Message translated');
