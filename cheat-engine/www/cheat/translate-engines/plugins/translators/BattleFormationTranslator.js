...
const CLASS_EFFECT_TAG_REGEX = /<(?:陣形効果テキスト追加|AddFormationEffectText)\s*:(.*?)>/i;
...
export class BattleFormationTranslator extends BasePluginTranslator {
    ...
    _isBattleFormationUiSceneActive() {
        ...
        const sceneName = typeof scene.constructor?.name === 'string' ? scene.constructor.name : '';
    ...
    _installWindowDrawHooks() {
        ...
        windowBaseProto.drawText = function (text) {
            ...
            if (
                ...
                !isBattleFormationUiSceneActive()
            ) {
