...
export class TagManager {
  ...
  preprocessTags(text) {
    ...
    for (const config of TAG_CONFIGS) {
      ...
      if (config.hasParam) {
        result = result.replace(config.prePattern, (match, letter, param) => {
          if (config.paramFormat === "angle") {
            caseMap.push({ type: config.type, case: letter });
            return `[b=${config.shortTag}<${param}>]`;
          }

          caseMap.push({ type: config.type, num: param, case: letter });
          return `[b=${config.shortTag}${param}]`;
        });
      } else {
  ...
  postprocessTags(text, tagCounts, caseMap) {
    ...
    for (const config of TAG_CONFIGS) {
      ...
      if (config.hasParam) {
        result = result.replace(config.postPattern, (match, param) => {
          if (config.paramFormat === "angle") {
            const originalCase = caseLookup[config.type] || config.defaultCase;
            return `\\${originalCase}<${param}>`;
          }

          const caseLookupForType = caseLookup[config.type] || {};
          const originalCase = caseLookupForType[param] || config.defaultCase;
          return `\\${originalCase}[${param}]`;
        });
      } else {
        let replaceIndex = 0;
    ...
    let valid;
    if (
      this.allowNewlineMismatch !== undefined
        ? this.allowNewlineMismatch
        : false
    ) {
      const requiredTypes = TAG_CONFIGS.filter(
        (c) => c.requiredConsistency,
      ).map((c) => c.type);
      valid = requiredTypes.every(
        (type) => actualCounts[type] === (tagCounts[type] || 0),
      );
    } else {
      valid = Object.keys(tagCounts).every(
        (key) => actualCounts[key] === (tagCounts[key] || 0),
      );
    }
