...
export class ValidationService {
    ...
    validateTranslatedMapShape(translatedMap, itemData) {
        ...
        for (const item of itemData) {
            const key = item.cacheKey || item.id;
            if (!(key in translatedMap)) {
                errors.push(`Missing key: ${key}`);
            } else if (typeof translatedMap[key] !== 'string') {
     ...
     */
    async validateResponseLanguage(validationText, targetLangName, sourceLangName, isBackgroundJob = false) {
        ...
        try {
            ...
            const result = await this.aiEngine.requestChatCompletion(payload, { isBackgroundJob });
    ...
    buildValidationTextFromMap(translatedMap, itemData) {
        ...
        for (const item of itemData.slice(0, 5)) {
            const key = item.cacheKey || item.id;
            if (key in translatedMap && typeof translatedMap[key] === 'string') {
                samples.push(translatedMap[key]);
            }
        }
