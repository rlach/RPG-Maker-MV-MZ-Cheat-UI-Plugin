 ...
 * Implements: resendFirstHalf, askAIToFix, useJsonFixer, none
 */
...
export class RetryHandler {
    ...
    async handleJsonError(options) {
        const { strategy, originalPayload, previousResponse, itemData, isBackgroundJob } = options;
    ...
    async retryTranslationWithError(payload, previousResponse) {
        try {
            ...
            feedbackPayload.messages = [
                ...
                {
                    role: 'user',
                    content: 'Wrong! Try again! Make sure ALL values are valid JSON strings and the output is valid JSON.'
                }
            ];
