...
export class SystemCommands extends BasePhase {
    ...
    collectUntranslated({ panel }) {
        if (
            !(window.$dataSystem && $dataSystem.terms && Array.isArray($dataSystem.terms.commands))
        ) {
            return [];
        }

        const sourceCommands = $dataSystem.terms.commandsOriginal || $dataSystem.terms.commands;
