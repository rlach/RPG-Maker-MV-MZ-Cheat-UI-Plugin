  ...
  async batchTranslate(items) {
    ...
    try {
      ...
      const payload = {
        q: htmlPayload,
        source:
          this.panel.sourceLang === "auto" ? "auto" : this.panel.sourceLang,
    ...
    const html =
      translatedHtml && translatedHtml.trim().length ? translatedHtml : null;
    ...
    if (!html) {
      console.warn(
        "[LibreTranslate] HTML batch returned empty/original, NOT caching",
      );
      items.forEach((item) =>
        failures.push({
          type: item.type,
    ...
    for (let idx = 0; idx < items.length; idx++) {
      ...
      if (doc) {
        const node = doc.querySelector(
          `span[data-t="${htmlType}"][data-i="${idx}"]`,
        );
        if (node) {
        ...
        const regex = new RegExp(
          `<span[^>]*data-t=\\"${htmlType}\\"[^>]*data-i=\\"${idx}\\"[^>]*>([\\\\s\\\\S]*?)<\\\\/span>`,
          "i",
        );
