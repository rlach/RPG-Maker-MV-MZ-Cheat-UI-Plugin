...
export class BatchSummaryReporter {
    ...
    static mergeErrorStats(target, errorStats) {
        const accumulator =
            target && typeof target === 'object' ? target : this.createErrorStatsAccumulator();
        ...
        for (const key of Object.keys(normalized.byType)) {
            accumulator.byType[key] = (accumulator.byType[key] || 0) + normalized.byType[key];
        }
    ...
    static buildProgress({
        ...
        const safeTotalProcessed = Math.max(0, Number(totalProcessed) || safeProcessed);
        const percent = safeTotal > 0 ? Math.round((safeProcessed / safeTotal) * 100) : 100;

        const message = `${safeProcessed}/${safeTotal} ${progressLabel} (${percent}%)`;

        let currentErrorsLine = null;
        if (safeCurrentErrors > 0) {
            const currentErrorsBase = Math.max(0, safeCurrentProcessed || safeProcessed);
    ...
    static buildSummary({
        ...
        const normalizedErrorStats = this.normalizeErrorStats(errorStats, safeFailures);
        const safeCurrentPhaseErrors = Math.max(0, this.toSafeNumber(currentPhaseErrors, 0));
        const safeTotalCumulativeErrors = Math.max(
            0,
            this.toSafeNumber(totalCumulativeErrors, normalizedErrorStats.totalErrors)
        );
        const percent = safeTotal > 0 ? Math.round((safeSuccess / safeTotal) * 100) : 100;
        ...
        if (safeCurrentPhaseErrors > 0) {
            const phaseErrorPercent =
                safeTotal > 0 ? Math.round((safeCurrentPhaseErrors / safeTotal) * 100) : 0;
            lines.push(
        ...
        if (safeTotalCumulativeErrors > 0) {
            const totalErrorPercent =
                safeSuccess + safeTotalCumulativeErrors > 0
                    ? Math.round(
                          (safeTotalCumulativeErrors / (safeSuccess + safeTotalCumulativeErrors)) *
                              100
                      )
                    : 0;
            lines.push(
        ...
        if (normalizedErrorStats.recoveredErrors > 0) {
            lines.push(`recovered errors using strategy: ${normalizedErrorStats.recoveredErrors}`);
        }

        if (normalizedErrorStats.recoveryAttempts > 0) {
            lines.push(`recovery strategy uses: ${normalizedErrorStats.recoveryAttempts}`);
        }
