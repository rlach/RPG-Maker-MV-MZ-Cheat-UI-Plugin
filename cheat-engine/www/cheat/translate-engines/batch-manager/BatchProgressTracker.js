...
export class BatchProgressTracker {
    ...
    resume(stepLabel) {
        ...
        if (this.prePauseSnapshot) {
            ...
            this.phaseStartQueueProcessed = this.prePauseSnapshot.phaseStartQueueProcessed;
            this.prePauseSnapshot = null;
        }
        if (stepLabel) {
    ...
    _render() {
        const title = this.paused
            ...
            : this.currentStepLabel;
        const totalCompletionLine = this.panel?.getOverallTranslationCompletionLine?.() || null;
