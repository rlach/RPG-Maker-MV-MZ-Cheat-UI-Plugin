...
- `registerPluginCustomTags()` is called when the plugin is detected, and it's used to register any custom tags for this plugin that can be used in translations. This is optional and only needed if the plugin has specific \command tags that are not covered by the default set. When plugin has tags ALWAYS register them, ALWAYS import TAG_TYPE, TAG_STYLE, TAG_BRACKET from ai-engine/constants.js - THEY EXIST FOR A REASON. By default set requiredConsistency to true. Mask only values which could break scripts when changed.
