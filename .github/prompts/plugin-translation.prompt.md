...
- `registerPluginCustomTags()` is called when the plugin is detected, and it's used to register any custom tags for this plugin that can be used in translations. This is optional and only needed if the plugin has specific \command tags that are not covered by the default set.
